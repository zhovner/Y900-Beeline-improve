...
define([ 'underscore', 'jquery', 'knockout', 'config/config', 'service', 'lib/jquery/chosen.jquery-min' ],
/**
 * Sms list
 * @module smslist
 * @class smslist
...
function(_, $, ko, config, service, chosen) {
	...
	function getSMSMessages(callback) {
		return service.getSMSMessages({location:1
			}, function(data) {
	...
	function groupSms(messages) {
        ...
        $.each(theSortedPeoples, function(s_i, sp){
            ...
            for (var i = 0; i < peoples[people].length; i++){
                ...
                if(peoples[people][i].tag == '6' && peoples[people][i].groupId == ''){ // 单条草稿
                    hasDraft = true;
                }
            }
	...
	function dealPhoneBooks(){
		...
		for(var i = 0; i < config.listMsgs.length; i++){
			...
			for(var j = 0; j < config.phonebook.length; j++){
                ...
                if (smsItem.itemId == getLast8Number(book.pbm_number)) {
                    smsItem.name = book.pbm_name;
                    for (var k = 0; k < options.length; k++) {
                        if (book.pbm_number == options[k].value) {
                            options[k].value = smsItem.number;
			...
			if($.inArray(smsItem.itemId, tmp) == -1){
				options.push(new Option(smsItem.number , smsItem.number));
	...
	deleteSelectClickHandler = function(){
	 ...
	 choseUsersDeleteSelectClickHandler = function () {
		$("#device-message input").each(function(){
			$(this).parent('p').addClass('checkbox_selected');
			$(this).attr('checked','checked');
		})
		if( $("#device-message input").length > 0){
			$("#disChoseLi").show();
			$("#choseLi").hide();
		}
	...
	sendSmsClickHandler = function(){
       ...
       // if(nums.length + smsCapability.nvUsed > smsCapability.nvTotal){
		 if(nums.length  > smsCapability.nvLeft){
            showAlert({msg: "sms_capacity_will_full_just", params: [smsCapability.nvLeft]});
            return;
        }
		
		if(getSmsCount(msgContent) > smsCapability.nvLeft){
            showAlert("sms_capacity_will_full_just2");
            return;
        }
	...
	getLatestMessage = function(){
		var data = service.getSMSMessages({location:1
		});
...
//					errorOverlay(error.errorText);
	...
	function getReadyStatus() {
		...
		var getPhoneBookReady = function () {
			var data = service.getPhoneBookReady({}, function (data) {				
	...
	function updateSmsCapabilityStatus(capabilityContainer, callback){
		service.getSmsCapability({}, function(capability){
          ...
          hasCapability = (capability.nvLeft != 0);
	...
	bindingEvents = function(){
        ...
        $("#name").die().live('keyup', function(){
            var max = 22;
            var encodeType_pbm = getEncodeType2($("#name").val());
            if("ru" == encodeType_pbm.encodeType){
                max = 23;
            }else{
                max = 26;
            }
            $("#name").removeAttr("maxlength").attr("maxlength",max);
        }).live("paste", function(){
                var max = 22;
                var encodeType_pbm = getEncodeType2($("#name").val());
                if("ru" == encodeType_pbm.encodeType){
	...
	function saveDraftAction(data){
		...
		var params = {
			index: -1,
			currentTimeString:getSystemTime()/*getCurrentTimeString(datetime)*/,
			groupId:datetime.getTime()/* data.numbers.length > 1 ? datetime.getTime() : ''*/,
		...
		function getLatestDraftSms(numbers){
			service.getSMSMessages({location:1			
			}, function(data) {
