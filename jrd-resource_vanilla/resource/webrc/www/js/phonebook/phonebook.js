...
define([ 'jquery', 'knockout', 'config/config', 'service', 'underscore', 'lib/jquery/chosen.jquery-min'],
    ...
    function ($, ko, config, service, _, chosen) {
        ...
        var templateColumns = {
            ...
            listColumns:[
                ...
                { columnType:"images", headerTextTrans:"save_location", rowText:"imgLocation", width:"23%", sortable:true},
        ...
        function phoneBookVM() {
            ...
            function getNameMaxLength() {
                ...
                if (self.selectedLocation() == locationValue.DEVICE) {
                    ...
                    //对"^"需要按照2个字符处理
                  if("ru" == encodeType.encodeType){
                      max =  self.capacity().simMaxNameLen - 3;
                      if(max > 23){
                          max = 23;
                      }
                      return  max;
            ...
            self.sendMessage = function(){
                service.getSmsCapability({}, function (capability) {
                    var hasCapability = capability.nvLeft <= 0;
                    if (hasCapability) {
                    ...
                    if (numbers.length  > capability.nvLeft) {
                        showAlert({msg: "sms_capacity_will_full_just", params: [capability.nvLeft]});
                        return false;
                    }
					
                    if(getSmsCount(self.messageContent()) > capability.nvLeft){
            ...
            function getPhoneBookReady() {
                service.getPhoneBookReady({}, function (data) {
                    ...
                    if (data.pb_init_state == PB_INIT_STATE_NOT_INIT) {
                        addTimeout(getPhoneBookReady, 2000);
            ...
            function preOpenEditPage() {
                config.resetContentModifyValue();
                setPreContent();
                $("#txtName").removeAttr("maxlength");
                $("#txtMobile").removeAttr("maxlength");
            ...
            if (books) {
                for (var i = 0; i < books.length; i++) {
                    ...
                    var temp = {
                        ...
                        imgLocation:books[i].pbm_location == locationValue.SIM ? "images/sim.png" : "images/device.png",
